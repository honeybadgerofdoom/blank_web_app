...
class KnightTest {
	...
	@Test
	void test_knight_at_top() {

			ChessPiece blackKnight = new Knight(testBoard, Color.BLACK);
			testBoard.placePiece(blackKnight, "f6");
			ArrayList<String> testList = new ArrayList<String>(Arrays.asList("g8", "e8", "d7", "d5", "e4", "g4", "h5", "h7" ));
			assertEquals(blackKnight.legalMoves().size(), testList.size());
			for (int i = 0; i < testList.size(); i++) {
				assertTrue(blackKnight.legalMoves().contains(testList.get(i)));
			}
